...
bad= []
if len (out) != 8: bad.append ('%d outline entries, not 8' % len (out))
if len (deep) != 1: bad.append ('no third level in the outline')
for n in ['#sec-first', '#sec-deep', '#sec-second', '#on-page-two', '#sec-été']:
...
# an address which is not ASCII, percent encoded (a URI is ASCII)
if not any ('https://fr.wikipedia.org/wiki/%C3%89t%C3%A9' in l for l in links):
    bad.append ('no link to https://fr.wikipedia.org/wiki/%C3%89t%C3%A9')
if not any (l.startswith ('outline: 2.2 Größe à Paris') or '2.2 Größe à Paris' in l for l in out):
    bad.append ('the accented title of the outline is wrong')
