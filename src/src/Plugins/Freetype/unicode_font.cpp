 ...
 //   tex_gyre_operators ();

  {
    ...
    if (!is_nil (f->mathtable)) {
      // we have a MATH table, switch to opentype math typesetting
      mathface= f;
...
unicode_font_rep::get_left_slope (string s) {
  ...
  if (N(c) >= 3) {
    if (is_math_italic (c))
      ...
      return 0.1;
    else if (math_type == MATH_TYPE_OPENTYPE && is_integral (s))
      return 0.1;
  }
  get_extents (c, ex);
...
unicode_font_rep::get_left_correction  (string s) {
  ...
  if (math_type == MATH_TYPE_OPENTYPE && is_integral (s))
    return - (((ex->x2 - ex->x1) / 16));
  else if (ex->x3 < ex->x1)
    return ex->x1 - ex->x3;
  return 0;
...
unicode_font_rep::get_ot_italic_correction (string s, bool left, SI& r) {
  if (math_type == MATH_TYPE_OPENTYPE) {
    ...
    if (mathface->mathtable->italics_correction_value->contains (glyph)) {
      ...
      r= (wfn*corr)/2000;
      cout << "ot_italic_corr [" << s << "] [" << corr << "] [" << r << "]" << LF;
      return true;
    }
  }
  return false;
