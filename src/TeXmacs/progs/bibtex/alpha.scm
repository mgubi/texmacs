  ...
  (do ((entry t (cdr entry)) (n 1 (+ n 1)))
      ...
      (if (func? (car entry) 'bib-entry)
          (let* ((label (bib-format-label-prefix 0 (car entry)))
                 (num (hash-table-ref bib-label-table label)))
            (hash-table-set! bib-key-table (list-ref (car entry) 2) label)
            (if num
                (hash-table-set! bib-label-table label
                           (if (equal? num `()) `(1 2) 
                               `(,@num ,(+ 1 (length num)))))
                (hash-table-set! bib-label-table label `()))))))

(define (bib-format-label n x)
  (let* ((pre (hash-table-ref bib-key-table (list-ref x 2)))
         (num (hash-table-ref bib-label-table pre)))
    (if (null? num) pre
        (with n (car num)
          (hash-table-set! bib-label-table pre (cdr num))
...
(tm-define (bib-sort-key x)
  ...
  (let* ((auths (bib-format-label-names (bib-field x "author")))
         (label (hash-table-ref bib-key-table (list-ref x 2)))
